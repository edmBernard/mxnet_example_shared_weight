...
trainer_shared = gluon.Trainer(net_shared.collect_params(), 'sgd', {'learning_rate': 0.1})
trainer_mod1 = gluon.Trainer(net_mod1.collect_params(), 'sgd', {'learning_rate': 0.1})
trainer_mod2 = gluon.Trainer(net_mod2.collect_params(), 'sgd', {'learning_rate': 0.1})
...
for e in range(epochs):
    ...
    print("after1: Epoch %s. Loss: %s, Train_acc %s, Test_acc %s" % (e, moving_loss, train_accuracy, test_accuracy))
...
for e in range(epochs):
    ...
    test_accuracy = evaluate_accuracy(test_data2, lambda x:net_mod1(net_shared(x)))
    train_accuracy = evaluate_accuracy(train_data2, lambda x:net_mod1(net_shared(x)))
    print("after2: Epoch %s. Loss: %s, Train_acc %s, Test_acc %s" % (e, moving_loss, train_accuracy, test_accuracy))
