...
act2 = get_all_symbols(data, label)
mlp_master = mx.mod.Module(symbol=act2, label_names=['softmax_label'], context=mx.cpu())
...
data = mx.sym.flatten(data=data)
label = mx.sym.Variable('softmax_label')
mlps1 = get_all_symbols(data, label)
mlp_model1 = mx.mod.Module(symbol=mlps1[0], label_names=['softmax_label'], context=mx.cpu())
...
data = mx.sym.flatten(data=data)
label = mx.sym.Variable('softmax_label')
mlps2 = get_all_symbols(data, label)
mlp_model2 = mx.mod.Module(symbol=mlps2[1], label_names=['softmax_label'], context=mx.cpu())
